...
def val_to_2_bytes(value: int) -> list:
    ...
    if len(value) > 16:
        raise ValueError("Value is bigger than 2 bytes!")
    val_to_n_bytes(value, 2)
...
def val_to_n_bytes(value: int, n_bytes: int) -> list:
    ...
    length = len(bin_val)
    if length > n_bytes * 8:
        raise ValueError("Value is bigger than specified number of bytes!")
    lst = []
    for i in range(length, 0, -8):
        if (i-8) < 0:
            lst.append(int(bin_val[0:i], 2))
            break
        lst.append(int(bin_val[i-8:i], 2))    
    return lst.reverse()
...
def main():
    print(val_to_n_bytes(430430, 3), "    Should Be     ", "[6, 145, 94]")
